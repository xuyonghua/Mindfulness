...
open class BannerLayoutManager : RecyclerView.LayoutManager(),
    ...
    private fun layoutItem(viewForPosition: View, offsetX: Int): Int {
        layoutDecoratedWithMargins(
            viewForPosition,
            offsetX,
            getItemTop(viewForPosition),
            offsetX + itemWidth,
            getItemTop(viewForPosition) + getDecoratedMeasuredHeight(viewForPosition)
        )
        return itemWidth
    }
    ...
    private fun scrollToRight(dx: Int, recycler: RecyclerView.Recycler): Int {
        ...
        while (true) {
            ...
            if (left + abs(dx) > paddingLeft) {
                ...
                layoutDecoratedWithMargins(
                    addView,
                    left - getDecoratedMeasuredWidth(addView),
                    getItemTop(addView),
                    left,
                    getItemTop(addView) + getDecoratedMeasuredHeight(addView)
                )
            } else {
                break
            }
        ...
        while (true) {
            ...
            if (decoratedRight - dx < mOrientationHelper.totalSpace) {
                ...
                layoutDecoratedWithMargins(
                    lastViewAdd,
                    decoratedRight,
                    getItemTop(lastViewAdd),
                    decoratedRight + getDecoratedMeasuredWidth(lastViewAdd),
                    getItemTop(lastViewAdd) + getDecoratedMeasuredHeight(lastViewAdd)
                )
            } else {
                break
            }
